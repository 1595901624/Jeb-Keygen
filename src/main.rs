...
fn parse_post(buf: &[u8]) -> std::io::Result<(Vec<u64>, Vec<u8>)> {
    ...
    Ok((
        vec![flag as u64, crc as u64, user_serial as u64, license_serial, machine_id],
...
/// long to int
pub fn long_to_int(long_data: u64) -> (u32, u32) {
...
fn main() -> std::io::Result<()> {
    println!("Please Input License Data:");
    let mut license_data = String::new();
    std::io::stdin().read_line(&mut license_data)?;
    // license test data
    ...
    // rc4 decrypt
    let rc4_key = (&buf[..8]).iter().chain(RC4_KEY_EXT.iter()).cloned().collect::<Vec<u8>>();
